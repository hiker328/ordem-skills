# Modelo de qualificação de lead (cliente de agência)

Score de 0–100 baseado em 6 critérios, pensado para qualificar **empresas que podem
contratar a agência**. Pontue só com evidência da conversa.

## Critérios e peso

| Critério | Peso | O que procurar |
|----------|------|----------------|
| **Dor** | 25 | Problema claro de marketing/vendas (sem leads, CAC alto, depende de indicação, agência anterior ruim)? Quão urgente? |
| **Fit / ICP** | 20 | É o cliente ideal da agência: segmento atendido, **tem produto/serviço que escala**, ticket que comporta a verba? |
| **Verba de mídia + fee** | 15 | Tem (ou pode ter) orçamento de tráfego **e** condição de pagar o fee? Reagiu a preço sem fugir? |
| **Decisor** | 15 | Fala com quem decide (dono/sócio) ou com influenciador? |
| **Urgência** | 15 | Tem prazo/gatilho (lançamento, meta, sazonalidade) vs "algum dia"? |
| **Engajamento** | 10 | Responde rápido, faz perguntas, pede proposta/próximos passos? |

Some os pontos obtidos (proporcional dentro de cada critério) para o score 0–100.

## Faixas

| Faixa | Score | Significado / ação |
|-------|-------|--------------------|
| 🔥 **Quente** | 75–100 | Pronto para avançar. Priorizar agora, puxar para fechamento. |
| 🟡 **Morno** | 50–74 | Tem potencial; falta resolver 1–2 critérios. Nutrir + descobrir. |
| 🧊 **Frio** | 25–49 | Interesse baixo ou muitas lacunas. Cadência de follow-up de valor. |
| ⛔ **Desqualificado** | 0–24 | Sem fit, sem dor ou sem condição. Não gastar energia do closer. |

## Sinais úteis

**Sinais de compra:** pergunta preço/prazo, fala de "quando começamos", envolve
outras pessoas da empresa, descreve a dor em detalhe, pede proposta.

**Red flags:** some por dias, só responde monossílabo, "só pesquisando", não tem
orçamento e não há gatilho, decisor inacessível, fora do ICP.

## Formato de saída (um lead)

```markdown
## Qualificação — <nome/contato>
**Score:** 0/100 — **Faixa:** 🔥/🟡/🧊/⛔

| Critério | Nota | Evidência |
|----------|------|-----------|
| Dor | 0/25 | "citação ou 'não identificado'" |
| Fit/ICP | 0/20 | ... |
| Orçamento | 0/15 | ... |
| Decisor | 0/15 | ... |
| Urgência | 0/15 | ... |
| Engajamento | 0/10 | ... |

**Sinais de compra:** ...
**Red flags:** ...
**Próxima ação recomendada:** <objetiva e específica>
```
